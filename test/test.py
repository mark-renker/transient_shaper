...
def safe_int(value):
    """Safely convert cocotb value to int, handling X/Z values"""
    try:
        return int(value)
    except ValueError:
        # If value contains X/Z, return 0 or handle appropriately
        return 0
...
async def test_transient_shaper(dut):
    ...
    dut.rst_n.value = 1
    # Wait longer for gate-level simulation to settle
    ...
    output1 = safe_int(dut.uo_out.value)
    ...
    await ClockCycles(dut.clk, 50)
    output2 = safe_int(dut.uo_out.value)
    ...
    await ClockCycles(dut.clk, 50)
    output3 = safe_int(dut.uo_out.value)
    ...
    peak_output = safe_int(dut.uo_out.value)
